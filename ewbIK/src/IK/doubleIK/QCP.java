...
public class QCP {
	...
	private SGVec_3d[] xref;
	private SGVec_3d[] yref;
	private SGVec_3d xtrans;
	private SGVec_3d ytrans;
	 ...
	 */
	private void set(SGVec_3d[] x, SGVec_3d[] y) {
		this.x = x;
		this.y = y;
	 ...
	 */
	private void set(SGVec_3d[] x, SGVec_3d[] y, double[] weight) {
		this.x = x;
		this.y = y;
		this.weight = weight;
		rmsdCalculated = false;
	 ...
	 * @return
	 */
	public Matrix4d weightedSuperpose(SGVec_3d[] fixed, SGVec_3d[] moved, double[] weight) {
	 ...
	 * @return
	 */
	private void innerProduct(SGVec_3d[] coords1, SGVec_3d[] coords2) {
	...
	private int calcRotationMatrix() {
		...
		rotmat.val[Matrix3d.M00] = a2 + x2 - y2 - z2;
		rotmat.val[Matrix3d.M01] = 2 * (xy + az);
		rotmat.val[Matrix3d.M02] = 2 * (zx - ay);

		rotmat.val[Matrix3d.M10] = 2 * (xy - az);
		rotmat.val[Matrix3d.M11] = a2 - x2 + y2 - z2;
		rotmat.val[Matrix3d.M12] = 2 * (yz + ax);

		rotmat.val[Matrix3d.M20] = 2 * (zx + ay);
		rotmat.val[Matrix3d.M21] = 2 * (yz - ax);
		rotmat.val[Matrix3d.M22] = a2 - x2 - y2 + z2;
	 ...
	 */
	public double getWeightedRmsd(SGVec_3d[] fixed, SGVec_3d[] moved, double[] weight) {
		set(moved, fixed, weight);
		return getRmsd();
	}
