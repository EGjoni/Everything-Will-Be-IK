...
public class StrandedArmature
{
	...
	public Rot getAverageTargetOrientationAcrossAllStrandsForBone(
		...
		double q0Sum = 0d, q1Sum = 0d, q2Sum= 0d, q3Sum =0d;
		...
		for(Strand s : strandsForBone) {
			...
			double weight = s.getStrandTip().getIKPin().getPinWeight();
			totalWeight += weight; 			
			...
			/**
			 * TODO: double check that this isn't idiotic.
			 */
			Quaternion repairedAxesTargetQ = G.getSingleCoveredQuaternion(G.getQuaternion(targetAxes), thisBoneQ);
	...
	public SGVec_3d getAverageTargetOriginAcrossAllStrandsForBone(
			AbstractBone forBone,
			boolean importanceWeighting) {
		...
		ArrayList<Strand> strandsForBone = boneStrandMap.get(forBone); 		
		for(Strand s : strandsForBone) {
			SGVec_3d origin = s.getStrandTip().getIKPin().getAxes().origin_();
			double weight = s.getStrandTip().getIKPin().getPinWeight();
			totalWeight += weight;
	...
	public Rot getAverageTipOrientationAcrossAllStrandsForBone(
		...
		for(Strand s : strandsForBone) {
			 ...
			 * TODO: double check that this isn't idiotic.
			 */
			Quaternion repairedAxesTipQ = G.getSingleCoveredQuaternion(G.getQuaternion(tipAxes), thisBoneQ);
	...
	public SGVec_3d getAverageTipOriginAcrossAllStrandsForBone(
			AbstractBone forBone,
			int pass, 
			boolean importanceWeighting) {
		SGVec_3d result = new SGVec_3d();
		...
		ArrayList<Strand> strandsForBone = boneStrandMap.get(forBone); 		
		for(Strand s : strandsForBone) {
			SGVec_3d origin = s.simulatedLocalAxes.get(s.getStrandTip())[pass].origin_();			
			double weight = s.getStrandTip().getIKPin().getPinWeight();
			totalWeight += weight;
			result.x += origin.x*weight; 
			result.y += origin.y*weight;
	...
	public class Strand
	{
		...
		public HashMap<AbstractBone, Double> remainingFreedomAtBoneMap ; 
		double distToTarget;
