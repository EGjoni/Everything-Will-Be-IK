...
public abstract class AbstractKusudama implements Constraint, Saveable {
	...
	public void setAxesToInverseLimitingAxes(
		...
		parentTipAxes.scaleYBy(-1f); parentTipAxes.updateGlobal();
		...
		localRelativeLimitAxes.translateTo(new SGVec_3f(0,0,0));
	...
	public void snapToInvertedLimit(
		...
		SGVec_3f simulatedLimitAxesOffset = SGVec_3f.sub(simulatedLimitAxes.origin_(), boneAxes.origin_());
		...
		invertedAxes.scaleYBy(-1f); invertedAxes.updateGlobal();
	...
	public boolean inInvertedOrientationLimit(SGVec_3f globalPoint, AbstractAxes simulatedInvertedLimitAxes_global) {
		if(orientationallyConstrained) {
			float[] inBounds = {-1f}; 
			pointInLimits(globalPoint, inBounds, simulatedInvertedLimitAxes_global);
			return inBounds[0] > -1f; 
		} else return true;
	}
	...
	public void updateInverseTwistAxes() {
		...
		inverseLimitingAxes.alignGlobalsTo(baseInverseLimitingAxes);
		Rot alignToBoneInverseY = new Rot(baseInverseLimitingAxes.y().heading(), attachedTo.localAxes().y().getRayScaledBy(-1f).heading());
	...
	public float signedAngleDifference(float minAngle, float base) {
		float d = Math.abs(minAngle - base) % G.TAU; 
		...
		if (limitCones.size() > 1 && this.orientationallyConstrained) {
			for (int i =0; i<limitCones.size() -1; i++) {
				...
				if(inSegBounds == false && (closestCollisionPoint == null 
						|| SGVec_3f.angleBetween(collisionPoint, point) < 
						SGVec_3f.angleBetween(point,closestCollisionPoint))) {
	...
	public SGVec_3f pointInSoftBound(SGVec_3f currentPosition_global, SGVec_3f targetPosition_global, AbstractAxes limitingAxes) {
		...
		SGVec_3f totalPenaltyAxis = new SGVec_3f(0f,0f,0f);
		float highestLimit = 1f;				
		float prevPathHeight = 0f;
		...
		for(int i=1; i < coneCount; i++) {
			...
			totalPenaltyAxis.add(tripletDirection.mult(1f - tripletSubtractionAmount));
		}
		System.out.println("PreSmoothPenalty coeffecient: " + (highestLimit));
		highestLimit = G.smoothstep(0f, 1f, highestLimit);
		...
		float penalizedAngle = desiredAngleAboutAxis * (1f-highestLimit);
	...
	protected void updateRotationalFreedom() {
		...
		rotationalFreedom = axialConstrainedHyperArea * ( isOrientationallyConstrained() ?  Math.min(totalLimitConeSurfaceAreaRatio, 1f) : 1f); 
	...
	public void setStrength(float newStrength) {
		this.strength = Math.max(0f, Math.min(1f, newStrength));
	}
